...
Este codigo contiene el algoritmo weighted A*

-Pasos a Seguir para Ejecutar el Codigo-
llamar la funcion weighted_a_sar en el main
'''''
#---Dependencias---
...
# Función para implementar el algoritmo Weighted A*
def weighted_a_star(graph, coordinates, start, goal, weight, ver_proceso):
    ...
    while frontier:
        ...
        for neighbor in graph[current_node].keys():
            ...
            if tentative_g_score < g_scores[neighbor]:
                ...
                f_value = tentative_g_score + (weight * h_value)  # Calcular el valor f(n) = g(n) + w * h(n)
                if ver_proceso == 'y':  #visualiza el proceso
                    print("\n Nodo = {}, Heuristica = {}".format(neighbor,f_value))
...
start = 'Cancun'
goal = 'Campeche'
weight = 2  # Peso para la heurística

shortest_path, shortest_distance = weighted_a_star(mexDirectedGraph, cities_coordinates, start, goal, weight)
